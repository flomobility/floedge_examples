...
from config import h264_folder, motion_threshold,log_dir,log_file,logwrite_th, connect_th
    ...
    def run(self):
        ...
        while not self.stoprequest.isSet():
            try:
                ...
                fname=h264_folder+h264_filename[:h264_filename.rfind('.')]+'.mp4'
                ...
                if self._connected() and len(self.files_not_sent)>0:
                    for f,now in self.files_not_sent:
                        try:
                            text="\nMotion detected in your room at {}. Please see attached video.\n".format(now)
